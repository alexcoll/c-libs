...
//
void llist_add(LinkedList * list, char * name, int value) {
	// Create new node
...
//
int llist_remove_value(LinkedList * list, int value) {
	ListNode * e;
	ListNode * previous;
...
int llist_remove_name(LinkedList * list, char * name) {
	ListNode * e;
	ListNode * previous;
...
//
int llist_get_ith(LinkedList * list, int ith, int * name, int * value) {
	ListNode * e;
...
int llist_remove_ith(LinkedList * list, int ith) {
	...
	while (e != NULL) {
		if (n == ith) {
			return llist_remove_name(list, e->name);
		}
		e = e->next;
		n++;
	}
...
int llist_read(LinkedList * list, char * file_name) {
	...
	while (fgets(buffer, sizeof(buffer), fp) != NULL) {
		...
		llist_add(list, name, atoi(value));
	}
	fclose(fp);
	return 1;
}
...
//
void llist_insert_first(LinkedList * list, char * name, int value) {
	ListNode * n = (ListNode *) malloc(sizeof(ListNode));
	n->name = name;
	n->value = value;
	n->next = list->head;
...
//
void llist_insert_last(LinkedList * list, char * name, int value) {
	ListNode * e;
